<?php
if (!class_exists('Optione_Page_Title')) {
    class Optione_Page_Title
    {
        public function get_page_title(){
            $pt_mode = optione()->get_opt('pt_mode');
            if( $pt_mode == 'none' ) return;

            $ptitle_layout = (int)optione()->get_opt('ptitle_layout');
            $titles = $this->get_title();

            $breadcrumb_on = optione()->get_opt( 'breadcrumb_on', false );
            $breadcrumb = new Optione_Breadcrumb();
            $entries = $breadcrumb->get_entries();
 
            $brc_divider = '<span class="br-divider pxli pxli-long-arrow-right rtl-flip"></span>';
        ...
        public function get_title() {
            $title = '';
            $single_post_title_layout = optione()->get_theme_opt('single_post_title_layout','0');
            $post_custom_title  = optione()->get_theme_opt('post_custom_title',esc_html__('Blog details', 'optione'));
            // Default titles
            if ( ! is_archive() ) {
                // Posts page view
                if ( is_home() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404', 'optione' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'optione' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                ...
                $custom_title = optione()->get_opt('custom_title');
            ...
            $main_title = optione()->get_page_opt('custom_main_title');
            if (!empty($main_title)){
                $title = $main_title;
            }
            $sub_title = optione()->get_opt('custom_sub_title');
            $page_sub_title = optione()->get_page_opt('custom_sub_title');
