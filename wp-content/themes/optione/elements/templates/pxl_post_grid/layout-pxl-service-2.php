...
$post_type            = $widget->get_setting('post_type','pxl-service');
$layout               = $widget->get_setting('layout_'.$post_type, 'pxl-service-1');
...
$load_more = array(
    ...
    'button_text'         => $widget->get_setting('button_text', 'Get Service'),
);

$widget->add_render_attribute( 'wrapper', [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-service-grid layout-'.$layout),
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    ...
    <?php if ($select_post_by === 'term_selected' && $filter == "true"): ?>
        <div class="grid-filter-wrap d-flex <?php echo esc_html($settings['filter_alignment']);?>">
    ...
    <div <?php pxl_print_html($widget->get_render_attribute_string('grid')); ?>> 
        <?php optione_get_post_grid($posts, $load_more); ?>
    </div>
