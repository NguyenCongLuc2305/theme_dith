...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'optione');
...
$merlin_setup = get_option( 'merlin_' . optione()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'optione');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'optione');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'optione');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'optione' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'optione' ); ?></p>
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'optione' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Optione_Admin_Plugins;
