    ...
    function pxl_share_this_all($args=[]){
        ...
        <span class="pxl-post-share col-auto">
            <span class="meta-inner">
                <a data-hint="<?php esc_attr_e( 'Share this post to', 'ecohub' ); ?>"
                    ...
                    <span><?php echo esc_html__( 'Share', 'ecohub' ) ?></span> </a>
            </span>
        </span>
        <?php  
        $html = ob_get_clean();
...
function pxl_admin_posts_filter_restrict_manage_posts($post_type,$which){
    if ($post_type == 'pxl-template'){
        $template_types = array(
            'header'       => esc_html__('Header', 'ecohub'), 
            'footer'       => esc_html__('Footer', 'ecohub'), 
            'mega-menu'    => esc_html__('Mega Menu', 'ecohub') 
        );
        $template_types = apply_filters('pxl_template_type_support',$template_types);
        ?>
        <select name="pxl_filter_template_type">
            <option value=""><?php esc_html_e('Select Type', 'ecohub'); ?></option>
            <?php
...
*/
add_filter( 'update_plugins_7iquid.com', 'pxl_update_plugins_api', 10, 4 );
function pxl_update_plugins_api($update, $plugin_data, $plugin_file, $locales){
    $url      = 'http://api.7iquid.com/';
    $http_url = $url;
