...
add_action( 'wp_ajax_optione_get_pagination_html', 'optione_get_pagination_html' );
add_action( 'wp_ajax_nopriv_optione_get_pagination_html', 'optione_get_pagination_html' );
function optione_get_pagination_html(){
    if (!check_ajax_referer('optione_nonce', 'security')) {
        wp_send_json(
            array(
                'status'  => false,
                'message' => esc_html__('Nonce error, please reload.', 'optione')
            )
        );
    }
    try{
        if(!isset($_POST['query_vars'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'optione'));
        }
        $query_vars = isset($_POST['query_vars']) ? $_POST['query_vars'] : null;
        $query = new WP_Query($query_vars);
        ob_start();
        optione()->page->get_pagination( $query,  true );
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
                'message' => esc_attr__('Load Successfully!', 'optione'),
                'data' => array(
                    'html' => $html,
...
add_action( 'wp_ajax_optione_get_filter_html', 'optione_get_filter_html' );
add_action( 'wp_ajax_nopriv_optione_get_filter_html', 'optione_get_filter_html' );
function optione_get_filter_html(){
    try{
        ...
        $settings['filter_default_title'] = !empty($settings['filter_default_title']) ? $settings['filter_default_title'] : esc_html__( 'All', 'optione' );
        ob_start();
...
add_action( 'wp_ajax_optione_load_more_product_grid', 'optione_load_more_product_grid' );
add_action( 'wp_ajax_nopriv_optione_load_more_product_grid', 'optione_load_more_product_grid' );
function optione_load_more_product_grid(){
    try{
        if(!isset($_POST['settings'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'optione'));
        }
        ...
        if($posts->have_posts()){
            ...
            wp_send_json(
                array(
                    'status' => true,
                    'message' => esc_html__('Load Post Grid Successfully!', 'optione'),
                    'data' => array(
                        'html'  => $html,
                        'paged' => $settings['paged'],
                        'posts' => $posts,
            ...
            wp_send_json(
                array(
                    'status' => false,
                    'message' => esc_html__('Load Post Grid No More!', 'optione')
                )
            );
        }
    }
...
add_action( 'wp_ajax_optione_load_more_post_grid', 'optione_load_more_post_grid' );
add_action( 'wp_ajax_nopriv_optione_load_more_post_grid', 'optione_load_more_post_grid' );
function optione_load_more_post_grid(){
    try{
        if(!isset($_POST['settings'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'optione'));
        }
        ...
        optione_get_post_grid($posts, $settings);
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
...
if(!function_exists('optione_get_post_grid')){
    function optione_get_post_grid($posts = [], $settings = []){
        ...
        switch ($layout) {
            case 'post-list-1':
                optione_get_post_list_layout1($posts, $settings, $args_m);
                break;
            case 'post-list-2':
                optione_get_post_list_layout2($posts, $settings, $args_m);
                break;
            case 'post-1':
                optione_get_post_grid_layout1($posts, $settings, $args_m);
                break;
            case 'post-2':
                optione_get_post_grid_layout2($posts, $settings, $args_m);
                break;
            case 'post-3':
                optione_get_post_grid_layout3($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-list-1':
                optione_get_pxl_case_study_list_layout1($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-1':
                optione_get_post_grid_pxl_case_study1($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-2':
                optione_get_post_grid_pxl_case_study2($posts, $settings, $args_m);
                break;
            case 'pxl-service-1':
                optione_get_post_grid_pxl_service1($posts, $settings, $args_m);
                break;
            case 'pxl-service-2':
                optione_get_post_grid_pxl_service2($posts, $settings, $args_m);
                break;
            default:
                return false;
                break;
...
function optione_get_post_list_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        $button_text = !empty($button_text) ? $button_text : esc_html__('Read more', 'optione');
        $increase = $key + 1;
        $data_settings = '';
        $animate_cls = '';
                ...
                <div class="row gx-lg-30 align-items-lg-center">
                    ...
                    if (has_post_format('quote', $post->ID)){
                                            ...
                                            <div class="meta-inner d-flex align-items-center hover-underline">
                                                <?php if($show_author == 'true') : ?>
                                                    <span class="post-author col-auto d-flex">
                                                        <span><?php echo esc_html__('By', 'optione')."&nbsp;"; ?> </span>
                                                ...
                                                <?php if($show_comment == 'true') : ?>
                                                    <span class="post-comments d-flex">
                                                        <a href="<?php echo get_comments_link($post->ID); ?>">
                                                            <span><?php comments_number(esc_html__('No Comments', 'optione'), esc_html__(' 1', 'optione'), esc_html__(' %', 'optione'), $post->ID); ?></span>
                                                        </a>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                        ...
                        <div class="item-content">
                            ...
                            <?php endif; ?>
                            <?php
                            if ($show_date == 'true' || $show_comment == 'true'){
                                ?>
                                <div class="post-metas">
                                    <div class="meta-inner d-flex align-items-center">
                                        ...
                                        if($show_date == 'true') : ?>
                                            <span class="post-date">
                                                <span class="day-month">
                                                    <span class="date-month"><?php echo get_the_date('F', $post->ID).","; ?></span>
                                        ...
                                        <?php if($show_comment == 'true') : ?>
                                            <span class="post-comments d-flex align-items-center">
                                                <a href="<?php echo get_comments_link($post->ID); ?>">
                                                    <span><?php comments_number(esc_html__('No Comments', 'optione'), esc_html__(' 1', 'optione'), esc_html__('Comments:&nbsp; %', 'optione'), $post->ID); ?></span>
                                                </a>
                                            </span>
                                        <?php endif; ?>
                                    </div>
...
function optione_get_post_list_layout2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
                                    ...
                                    <div class="meta-inner">
                                        <?php if($show_author == 'true') : ?>
                                            <div class="post-author d-flex align-items-center">
                                                <span><?php echo esc_html('POST:', 'optione');?></span>
                                                <span>
                                                    <?php echo esc_html__('By', 'optione'); ?>&nbsp;<?php the_author_posts_link(); ?>
                                                </span>
                                        ...
                                        <?php if($show_comment == 'true') : ?>
                                            <div class="post-comments d-flex align-items-center">
                                                <span><?php echo esc_html('COMMENTS:', 'optione');?></span>
                                                <a href="<?php echo get_comments_link($post->ID); ?>">
                                                    <span><?php comments_number(esc_html__('No Comments', 'optione'), esc_html__(' 1', 'optione'), esc_html__(' % Comments', 'optione'), $post->ID); ?></span>
                                                </a>
                                            </div>
                                        ...
                                        <?php if($show_category == 'true') : ?>
                                            <div class="post-categories d-flex align-items-center hover-underline">
                                                <span><?php echo esc_html('CATEGORIES:', 'optione');?></span>
                                                <span class="post-category d-flex"><span><?php the_terms( $post->ID, 'category', '', ', ', '' ); ?></span></span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
...
function optione_get_post_grid_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        <div class="<?php echo esc_attr($str_item_class. ' '. $animate_cls . ' '. $filter_class); ?>" <?php pxl_print_html($data_settings); ?>>
            <div class="grid-item-inner">
                 <div class="grid-img-category">
                    <?php if (has_post_thumbnail($post->ID) && isset($thumbnail)): ?>
                        <div class="item-featured">
                            <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo wp_kses_post($thumbnail); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if($show_category == 'true') : ?>
                        <span class="post-category d-flex"><?php the_terms( $post->ID, 'category', '', ' ', '' ); ?></span>
                    <?php endif; ?> 
                </div>
                <div class="item-content">
                    ...
                    <?php endif; ?>
                    <h3 class="item-title"><a href="<?php echo esc_url(get_permalink( $post->ID )); ?>">
                       <?php if(is_sticky($post->ID)) { ?>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M17.2188 0.9375H2.78125C1.67383 0.9375 0.8125 1.83984 0.8125 2.90625V17.3438C0.8125 18.4512 1.67383 19.3125 2.78125 19.3125H12.8301C13.5273 19.3125 14.1836 19.0664 14.6758 18.5742L18.4082 14.8418C18.9004 14.3496 19.1875 13.6934 19.1875 12.9961V2.90625C19.1875 1.83984 18.2852 0.9375 17.2188 0.9375ZM3.4375 3.5625H16.5625L16.5215 12.75H13.9375C13.1992 12.75 12.625 13.3652 12.625 14.0625V16.6875H3.4375V3.5625Z" fill="#189195"/>
                        </svg>
                    <?php } ?>
                        <?php echo esc_attr(get_the_title($post->ID)); ?></a></h3>
                    <?php if($show_excerpt == 'true'): ?>
                        <div class="item-excerpt">
                            <?php
                                if(!empty($post->post_excerpt)){
...
function optione_get_post_grid_layout2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        <?php 
            if($key == 0 || $key == 3 || $key == 6 || $key == 9 || $key == 12){
               $str_item_class = 'grid-item col-xxl-8 col-xl-8 col-lg-12 col-md-12 col-sm-12 col-12'; 
            }else{
                $str_item_class = 'grid-item col-xxl-4 col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12 item-sidebar';
            }
        ...
        <div class="<?php echo esc_attr($str_item_class. ' '. $animate_cls . ' '. $filter_class);?>" <?php pxl_print_html($data_settings); ?>>
            ...
            <div class="grid-item-inner">
                ...
                <div class="item-content">
                    ...
                    <?php } ?>
                        <?php echo esc_attr(get_the_title($post->ID)); ?></a></h3>
                    <?php if($show_excerpt == 'true' && $key == 0 || $key == 3 || $key == 6 || $key == 9 || $key == 12): ?>
                        <div class="item-excerpt">
                            <?php
                                if(!empty($post->post_excerpt)){
                                    echo wp_trim_words( $post->post_excerpt, $num_words, null );
...
function optione_get_post_grid_layout3($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        if(!empty($args_m[$key]['thumbnail'])){
            ...
            if (has_post_thumbnail($post->ID) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)){
                ...
                if($img_id){
                    ...
                    $thumbnail = $img['thumbnail'];
                    $thumbnail_src = get_the_post_thumbnail($post->ID, $img_size);
                }else{
                    $thumbnail = get_the_post_thumbnail($post->ID, $img_size);
                }
            }
        ...
        <div class="<?php echo esc_attr($str_item_class. ' '. $animate_cls . ' '. $filter_class); ?>" <?php pxl_print_html($data_settings); ?>>
            <div class="grid-item-inner">
                 <div class="grid-img-category">
                    <?php if (has_post_thumbnail($post->ID) && isset($thumbnail) && !empty($img['url']) ): ?>
                        <div class="item-featured">
                            <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo wp_kses_post($thumbnail); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (empty($img['url']) ): ?>
                        <div class="item-featured pxl-thumbnail-src">
                            <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo wp_kses_post($thumbnail_src); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if($show_category == 'true') : ?>
                        <span class="post-category d-flex"><?php the_terms( $post->ID, 'category', '', ' ', '' ); ?></span>
...
function optione_get_pxl_case_study_list_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
...
function optione_get_post_grid_pxl_case_study1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
...
function optione_get_post_grid_pxl_case_study2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
...
function optione_get_post_grid_pxl_service1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
                ...
                <div class="item-content">
                    <div class="content-inner">
                        <div class="content-number">
                          <h4 class="item-title"><a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo esc_attr(get_the_title($post->ID)); ?></a></h4>
                          <div class="item-number"><?php echo esc_attr(sprintf("%02d", $increase)) ?> </div>
                        </div>
                        <?php
                        if (isset( $thumbnail )){
                            ?>
                        ...
                        if (!empty($area_icon['url']) && $icon_type == 'icon'){
                            ?>
...
function optione_get_post_grid_pxl_service2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
            ...
            <div class="grid-item-inner">
                <div class="item-content">
                    <div class="item-icon-content">
                        ...
                        if (!empty($area_icon) && $icon_type == 'icon'){
                            ?>
                            <div class="area-icon-wrap">
                                <div class="area-icon"><i class="<?php echo esc_attr($area_icon); ?>"></i></div>
                            </div>
