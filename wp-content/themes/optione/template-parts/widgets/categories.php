 ...
 * Custom HTML output
*/
if(!function_exists('optione_widget_categories_args')){
    add_filter('widget_categories_args', 'optione_widget_categories_args');
    add_filter('woocommerce_product_categories_widget_args', 'optione_widget_categories_args');
    function optione_widget_categories_args($cat_args){
...
class Cetara_Categories_Walker extends Walker_Category {
    ...
    public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $args['feed_image'] ) || ! empty( $args['feed'] ) ) {
            ...
            if ( empty( $args['feed'] ) ) {
                $alt = ' alt="' . sprintf(__( 'Feed for all posts filed under %s','optione' ), $cat_name ) . '"';
            } else {
                $alt = ' alt="' . $args['feed'] . '"';
                $name = $args['feed'];
...
if(!function_exists('optione_woocommerce_layered_nav_term_html')){
    add_filter('woocommerce_layered_nav_term_html', 'optione_woocommerce_layered_nav_term_html', 10, 4);
    add_filter('woocommerce_layered_nav_count', function (){ return '';});
    
    function optione_woocommerce_layered_nav_term_html($term_html, $term, $link, $count){
